...
`decode_block(...)`は`Block`を生成している関数だが、`if`の次が`0x40`の場合は戻り値なしを意味、それ以外はそのまま戻り値の数の値になる。
...
もし、こういうのも動かしたいなどの要望があればぜひスクラップに書いてもらえると嬉しい。
気が向くと書くかもしれない。
