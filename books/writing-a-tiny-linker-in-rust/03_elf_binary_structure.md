...
ELFファイルは大まかに図1のような構造をしている。
オブジェクトファイル（`.o`）と実行ファイル（`a.out`）で含まれるものは似ているが、それぞれ役割の異なるテーブルが「要」になる点に注目。

![](/images/writing-a-tiny-linker-in-rust/ch03-01-obj-vs-exec.png)
*図1*
...
シンボル名やセクション名は、図2のように、この中のオフセットとして参照される。

![](/images/writing-a-tiny-linker-in-rust/ch03-04-string-table.png)
*図2*
...
このようなビット詰め込み方式は、本章のあとで出てくる再配置エントリの`r_info`でも同じパターンが使われている。`st_info`（1バイト）と`r_info`（8バイト）のビット分割を1枚にまとめると図3のとおりとなる。

![](/images/writing-a-tiny-linker-in-rust/ch03-03-bit-encoding.png)
*図3*
...
セグメントは、OSがプログラムをメモリにロードするときの単位である。
1つのセグメントは複数のセクションを束ねたものになることが多い。
たとえばコード領域のセグメントには`.text`だけでなく`.rodata`も入る、といった具合である。

なぜセグメントが必要かというと、OSがメモリ保護属性を設定する単位が **ページ単位** で、セクションのままだと粒度が細かすぎるからである。
セグメントとしてまとめておけば「このセグメントは読み取り・実行のみ」「このセグメントは読み書きのみ」のように、ページ単位で揃った権限を割り当てられる。

オブジェクトファイル（`.o`）にはセグメントの情報はなく、セクションだけがある。
リンカーがリンクするときに、セクションを束ねてセグメントを組み立て、その情報をプログラムヘッダーとして実行ファイルに書き込む、という流れになる。

本書で生成する`a.out`のプログラムヘッダーは2つの **LOADセグメント** からなる。
1つ目は`.text`を含むコード領域（`R E` = 読み取り + 実行）、2つ目は`.data`を含むデータ領域（`RW` = 読み取り + 書き込み）である。
権限の組み合わせは後述の「セグメントフラグ」表で詳しく見ていく。

セクション・セグメント・メモリページの3層の関係をまとめると図4のとおりとなる。

![](/images/writing-a-tiny-linker-in-rust/ch03-02-section-and-segment.png)
*図4*
...
前節「セクションとセグメント」で触れたとおり、1つ目のLOADセグメント（`R E` = 読み取り + 実行）は`.text`を束ねたコード領域、2つ目のLOADセグメント（`RW` = 読み取り + 書き込み）は`.data`を束ねたデータ領域に対応する。
リンカー（GNU `ld`や本書の自作リンカー）は、これらの権限の組み合わせがちゃんと分かれるようにセクションを配置する責任を持つ。
