...
ELFには「**セクション**」と「**セグメント**」という2種類の見方がある。
これまで見てきた`.text`や`.data`などはセクションで、これは **リンク時のビュー** である。
リンカーが「どのコードをどこに集めるか」「どのシンボルがどこに属するか」を扱う単位として、細かく分けてある。

一方のセグメントは **実行時のビュー** で、OSがプログラムをメモリにロードするときの単位である。
1つのセグメントは複数のセクションを束ねたものになることが多い。
たとえばコード領域のセグメントには`.text`だけでなく`.rodata`も入る、といった具合である。

なぜ別のビューが必要かというと、OSがメモリ保護属性を設定する単位が **ページ単位** で、セクションのままだと粒度が細かすぎるからである。
